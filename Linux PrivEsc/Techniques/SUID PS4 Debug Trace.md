...
- `[+ ]` → `<ps4_ok>` = no (bash sanitizes `PS4` at startup; patched). Technique inapplicable → return to [[Linux Privilege Escalation Checksheet]] Step 8.
...
- None → no drop site → return to [[Linux Privilege Escalation Checksheet]] Step 8.
